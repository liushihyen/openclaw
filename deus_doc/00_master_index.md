...
本系列報告共十二份，涵蓋從專案偵察到行動指引的完整逆向工程分析，以及 Memory 子系統的深度專題解析。建議依序號閱讀，或依個人需求跳至特定報告。
...
### 🧠 Memory 子系統深度分析（07–12）

| 編號 | 文件名稱 | 分析階段 | 摘要 |
|------|---------|---------|------|
| 07 | [記憶體機制探索](07_memory_mechanism_discovery.md) | 機制探索 | 43 個檔案清單、概念定義（MemorySource、MemoryChunk）、CRUD 進入點 |
| 08 | [記憶體架構分析](08_memory_architecture_analysis.md) | 架構分析 | 七層架構、6 張 SQLite 資料表、儲存後端、完整生命週期、索引策略 |
| 09 | [記憶體資料流與整合](09_memory_data_flow_and_integration.md) | 資料流追蹤 | 寫入/索引/搜尋/讀取四條資料流路徑、8 個跨子系統整合點、事件驅動行為 |
| 10 | [記憶體設定與邊界條件](10_memory_configuration_and_constraints.md) | 設定與限制 | 50+ 個設定參數、容量限制、6 類邊界條件、多層降級策略 |
| 11 | [記憶體依賴關係對映](11_memory_dependency_mapping.md) | 依賴對映 | 內部依賴圖、外部/可選依賴、API 依賴、被依賴關係、健康度評估 |
| 12 | [記憶體逆向工程彙整](12_memory_reverse_engineering_summary.md) | 彙整摘要 | 架構洞察、關鍵發現、值得學習的設計、潛在風險、快速上手指南 |
...
| 1.1 | 2026-02-08 | 新增 Memory 子系統深度分析六份報告（07–12） |
